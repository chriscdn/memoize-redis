...
var RedisAdapter = class {
	...
	async set({ hash, field, value, ttl }) {
		await this.redis.multi().hSet(hash, field, value).hpExpire(hash, field, ttl).exec();
	}
	async get({ hash, field }) {
		const [value, _ttl] = await this.redis.multi().hGet(hash, field).hpTTL(hash, field).exec();
...
//#endregion
//#region src/index.ts
const createRedisMemoizer = (redisClient) => (cb, options) => {
...
* @returns
*/
const createRedisMemoizerNoHash = (redisClient) => (cb, options) => {
